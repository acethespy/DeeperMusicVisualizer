...
def step5_predict_notes(model, npz, params):
    ...
    #is_obj_pred is every time interval here the predicted value is > the borderline.
    #this means its not a 3d array but 2d array (instead of time segments x time interval x 5, the first 2 dims are collapsed)
    print(is_obj_pred.shape)
    ...
    print("others",others_pred.shape, others_pred[1])
    another_pred_result = np.concatenate([is_obj_pred, is_obj_pred * obj_type_pred, others_pred], axis=1);
    print("another",another_pred_result.shape)
    print("anoter", another_pred_result[1])
    print("{} notes predicted.".format(np.sum(is_obj_pred)));

    print(is_obj_pred.shape,another_pred_result.shape,timestamps.shape,ticks.shape,div_data.shape)
    print(timestamps[:5])
    print(ticks[:5])
    print(len([i for i in another_pred_result if i[2] == 1]))
...
def step5_convert_sliders(data, params):
    ...
    print(objs.shape, predictions.shape, ticks.shape, timestamps.shape, is_slider.shape, is_spinner.shape, is_note_end.shape, sv.shape, slider_ticks.shape)
    print("slider preds", predictions[2])
    print("slider ticks",slider_ticks[:5])
    print("timestamps", timestamps[:5])
    print("ticks", ticks[:5])
    print(objs[2],ticks[2],timestamps[2], is_slider[2],is_spinner[2], is_note_end[2])
