...
def prefilter_data(train_data_unfiltered, div_data_unfiltered, train_labels_unfiltered):
    ...
    #if UNUSED or IS_NOTE_END, then don't add to train data...JK the if True makes every datapt used
    #so train data and train_data unfiltered are the exact same
    #Train labels is as stated above, where the filtered version is just the first 0,1,2,3,4 labels (not including UNUSED).
    #X is every 1/4 beat
    ...
    print("daata",train_data_unfiltered.shape,train_data.shape)
    print("label",train_labels_unfiltered.shape,train_labels.shape)
...
def preprocess_npzs(train_data_unfiltered, div_data_unfiltered, train_labels_unfiltered):
    ...
    #given time_intervals = 16, divide first dimension X into (X//16,16,...) for training
    ...
    print("preprocessed data", train_data2.shape)
    print("preprocessed div", div_data2.shape)
    print("preprocessed label", train_labels2.shape)
...
def step2_train_model(model, PARAMS):
    ...
    if len(train_file_list) < too_many_maps_threshold:
        ...
        print("final train data AFTER validation split",new_train_data.shape)
