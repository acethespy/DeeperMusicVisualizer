...
class LucidSonicDream:
  ...
  def load_specs(self):
    ...
    self.osu = self.osu_file is not None
    if self.osu:
      self.osu_pulse, self.osu_motion = extract_osu(self.osu_file, len(self.spec_norm_class), frame_duration, sr_class)
      print(self.osu_pulse)
  ...
  def generate_vectors(self):
    ...
    osu_motion_react = OSU_MOTION_REACT * 20 / fps
    osu_pulse_react = OSU_PULSE_REACT
    ...
    pulse_base = np.ones(self.input_shape)
    motion_base = np.ones(self.input_shape)
    ...
    for i in range(len(self.spec_norm_class)):
      ...
      pulse_noise_add =  pulse_base * self.spec_norm_pulse[i] * self.pulse_react
      motion_noise_add = motion_base * self.spec_norm_motion[i] * motion_react * \
                         self.motion_signs * rand_factors
      if self.osu:
        pulse_noise_add = pulse_base * max(self.spec_norm_pulse[i] * self.pulse_react, self.osu_pulse[i] * osu_pulse_react)
        motion_noise_add = motion_base * max(self.spec_norm_motion[i] * motion_react, self.osu_motion[i] * osu_motion_react) * self.motion_signs * rand_factors
